...
import { GET_WORKTYPE } from 'graphql/queries/form';
...
const Worktype = (props: {fieldProperties: any; id: any}) => {
  ...
  //get contacts
  const { loading, error, data, refetch } = useQuery(GET_WORKTYPE, {
    variables: {
      "first": 10,
      "page": 1,
      "hasOrganization": {
  ...
  React.useEffect(() => {
    ...
    data?.workTypes.data.map((item: any, key: number) => {
      items_array.push({
        id: key, 
        uuid: key,
